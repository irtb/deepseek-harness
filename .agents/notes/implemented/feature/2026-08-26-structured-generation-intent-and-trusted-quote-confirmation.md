...
The structured context is authenticated user intent, not authorization or pricing authority. Laravel continues to validate the model and parameters and owns quotes, credits, refunds, and generation idempotency. Raw URLs, paths, file bytes, and unscoped attachments are rejected by the Gateway schema; protocol `2026-08-26.2` adds the scoped media IDs described in [Scoped image media references](2026-08-26-scoped-image-media-references.md).
...
Visible scalar settings reach Laravel quoting without model drift, and the approval amount always comes from the same quote Laravel will execute. A Gateway restart discards pending quote registry entries, so an interrupted confirmation requires a fresh quote; this fail-closed behavior is intentional. Protocol `2026-08-26.1` remains the scalar-only compatibility version, while [protocol `2026-08-26.2`](2026-08-26-scoped-image-media-references.md) owns scoped image media references and the current rolling-release order.
