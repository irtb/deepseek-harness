...
结构化上下文只代表已经认证的用户意图，不代表授权或报价权威。Laravel 继续校验模型和参数，并独占报价、扣分、退款及生成幂等。Gateway Schema 拒绝原始 URL、路径、文件字节和附件。
...
可见标量设置现在不会因模型漂移而偏离 Laravel 报价，批准金额始终来自 Laravel 将要执行的同一个报价。滚动发布顺序为先 Agent、后 Canvas：新 Canvas 声明 `2026-08-26.1`，新 Agent 为未声明版本的旧客户端投影 `2026-08-25.1` 响应。Gateway 重启会丢弃待确认报价注册记录，因此中断的确认必须重新报价；这是有意的安全失败行为。附件在独立的范围化媒体引用合同完成前保持禁用。
