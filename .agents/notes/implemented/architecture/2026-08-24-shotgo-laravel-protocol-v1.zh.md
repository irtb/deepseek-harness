...
在 `apps/shotgo-agent/contracts/` 中冻结 wire 版本 `2026-08-24`。Laravel 是唯一业务权威。Harness 推理使用专用的内部 inference SSE 端点；文本、图片、视频和音频的业务生成使用 Capability 端点，二者不得混淆。
...
浏览器到 Runtime 的交接使用一次性票据；Runtime 以服务身份交换得到不透明的短期 Capability Grant。Runtime 不把解码出来的 Token 内容当作权限权威。
