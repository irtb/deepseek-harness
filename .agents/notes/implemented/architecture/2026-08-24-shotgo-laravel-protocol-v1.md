...
Freeze wire version `2026-08-24` in `apps/shotgo-agent/contracts/`. Laravel is the sole business authority. Harness reasoning uses a dedicated internal inference SSE endpoint; text/image/video/audio business generation uses capability endpoints and is never confused with inference.
...
The browser-to-runtime handoff uses a single-use ticket exchanged under service authentication for an opaque short-lived capability grant. The runtime does not treat decoded token content as permission authority.
