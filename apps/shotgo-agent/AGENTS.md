...
- Complete every applicable check, preflight the remote branch topology, report the exact evidence plus the source, integration branch, production branch, and remote, then stop for one second explicit confirmation. That single confirmation authorizes the complete named merge-and-push sequence through the production branch; never split it into repeated confirmations for `master` and `release/*`. A changed or unusable target invalidates the proposal instead of authorizing a substitute. After the push, provide exact production deployment, verification, and rollback commands, but never execute production deployment.
