...
- Before implementation, identify shared contracts, tools, policies, callers, jobs, and data paths that could make the requested change regress another Agent or ShotGo service. Complete focused and affected-consumer checks before merging or releasing. No standing second-confirmation gate applies; merge, push, and deployment may proceed when included in the current request, while development-only scope never implies production mutation.
- Database and table changes have no mandatory second-confirmation or manual-only rule. Before execution, prominently report the exact migration or SQL, affected schemas, tables, rows, services, compatibility risks, backup, rollback, and validation; validate locally when available and mutate production only when the current task authorizes deployment.
