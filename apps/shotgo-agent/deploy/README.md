...
- runtime state and logs: `/data/projects/agent.shotgo.cn/storage`, with durable sessions under `storage/sessions`;
...
3. Back up any existing Agent files, place the artifact in `/data/projects/agent.shotgo.cn`, create `storage/logs`, `storage/dsh`, and `storage/sessions`, and grant only those runtime directories to `www-data`.
4. Install `/etc/shotgo-agent/shotgo-agent.env` with `SHOTGO_ENABLE_TRAFFIC=false`, the exact `https://canvas.shotgo.cn` browser origin, and `SHOTGO_AGENT_SESSION_ROOT=/data/projects/agent.shotgo.cn/storage/sessions`; keep only the Laravel service token outside the repository. Production startup fails closed when the session root is missing. Laravel supplies the encrypted-at-rest Ark credential and provider endpoint IDs and introspects opaque browser Grants through no-store internal endpoints.
