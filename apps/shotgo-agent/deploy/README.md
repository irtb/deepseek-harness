...
4. Install `/etc/shotgo-agent/shotgo-agent.env` with `SHOTGO_ENABLE_TRAFFIC=false`; keep `ARK_API_KEY` and the Laravel service token outside the repository.
...
8. Keep readiness closed until Ark direct-inference, Laravel policy/usage control-plane, and full session acceptance pass together.
